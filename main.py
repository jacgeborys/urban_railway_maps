...
from osm_config import build_overpass_query, classify_route, classify_stop, VISUAL_CONFIG
...
def process_routes(data, transformer):
    ...
    all_nodes = {el["id"]: transformer.transform(el["lon"], el["lat"]) for el in data["elements"] if el["type"] == "node"}
    ...
    for el in data["elements"]:
        if el["type"] == "relation":
            ...
            if category and category in lines_by_type:
                for member in el.get("members", []):
                    if member["type"] == "way" and member["ref"] in ways:
                        ...
                        if len(coords) >= 2:
                            lines_by_type[category].append(coords)
...
def process_stops(stops_data, transformer):
    ...
    for base_name, stop_list in all_stops.items():
        if len(stop_list) == 1:
            ...
            resolved_stops = resolve_stop_conflicts(stop_list, base_name)
            for stop in resolved_stops:
                add_to_category(stop, stop_dicts)
...
def resolve_stop_conflicts(stop_list, base_name):
    ...
    if len(types) == 1:
        return [{'coord': calculate_centroid([s['coord'] for s in stop_list]), 'type': types[0], 'original_name': base_name}]
    ...
    for stop_type in types:
        ...
        result.append({'coord': calculate_centroid([s['coord'] for s in type_stops]), 'type': stop_type, 'original_name': base_name})
    return result
...
def add_to_category(stop, stop_dicts):
    ...
    if name not in stop_dicts[stop_type]:
        stop_dicts[stop_type][name] = []
...
def find_transfers(stop_lists):
    ...
    all_stadiums = []
    type_names = ['tram', 'light_rail', 'metro', 'train']
    for (i, type1), (j, type2) in itertools.combinations_with_replacement(enumerate(type_names), 2):
        stops1, stops2 = stop_lists[i], stop_lists[j]
        key = '_'.join(sorted([type1, type2]))
    ...
    stadium_polygons = [create_stadium_polygon(s) for s in all_stadiums]
    stadium_polygons = [p for p in stadium_polygons if p and p.is_valid and not p.is_empty]
...
def create_stadium_polygon(stops):
    ...
    try:
        ...
        length = distance + TRANSFER_VISUAL_CONFIG['stadium_length_padding']
        width = TRANSFER_VISUAL_CONFIG['stadium_width']
        radius = width / 2
        angle = np.arctan2(p2.y - p1.y, p2.x - p1.x)
        rect_half_length = max(0, length - width) / 2
        points = np.array([(-rect_half_length, radius), (rect_half_length, radius), (rect_half_length, -radius), (-rect_half_length, -radius)])
...
def plot_transit_map(lines_by_type, stops_by_type, transfers, city_config):
    ...
    for line_type in ['train', 'metro', 'light_rail', 'tram']:
        ...
        for line in lines_by_type.get(line_type, []):
            xs, ys = zip(*line)
            ax.plot(xs, ys, color=config['color'], linewidth=config['linewidth'], alpha=config['alpha'], zorder=config['zorder'])
        if stops_by_type.get(line_type):
            xs, ys = zip(*stops_by_type[line_type])
            ax.scatter(xs, ys, c=config['stop_color'], s=config['stop_size'], marker=config['stop_marker'], zorder=config['zorder'] + 5)
    from matplotlib.patches import Polygon as MPLPolygon
    transfer_config = VISUAL_CONFIG['transfer']
    for poly in transfers:
        if hasattr(poly, 'exterior'):
            ax.add_patch(MPLPolygon(list(poly.exterior.coords), facecolor=transfer_config['facecolor'], edgecolor=transfer_config['edgecolor'],
                                    linewidth=transfer_config['linewidth'], alpha=transfer_config['alpha'], zorder=transfer_config['zorder']))
    ax.set_aspect('equal')
    ax.axis('off')
    ...
    ax.text(0.02, 0.98, city_config['name'], transform=ax.transAxes, fontsize=20, fontweight='bold', va='top', ha='left',
...
def generate_city_map(city_key):
    ...
    try:
        ...
        for data in [tram_light_rail_data, metro_data, train_data]:
            for key, lines in process_routes(data, transformer).items():
        ...
        for t in ['tram', 'light_rail', 'metro', 'train']:
            print(f"Found {len(lines_by_type[t])} {t} route segments and {len(stops_by_type[t])} stops.")
        transfers = find_transfers(list(stops_by_type.values()))
        print(f"Found {len(transfers)} transfer areas.")
        fig = plot_transit_map(lines_by_type, stops_by_type, transfers, city_config)
